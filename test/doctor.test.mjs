...
test("doctor warns when eval assertion ids are not E<number>", async () => {
  const root = await makeProject({
    specBody: specWith("node", "eval: .harness/eval.json\n"),
    files: {
      ".harness/eval.json": JSON.stringify({
        assertions: [{ id: "E1" }, { id: "C7" }, { id: "C8" }],
      }),
    },
  });
  ...
  const check = report.checks.find(c => c.name === "eval assertion ids");
  assert.equal(check.status, "warn");
  assert.match(check.detail, /C7, C8/);
  // A legacy id still grades; it only costs repair-prompt correlation.
  assert.equal(report.passed, true);
});

test("doctor accepts an all-E evaluate checklist", async () => {
  const root = await makeProject({
    specBody: specWith("node", "eval: .harness/eval.json\n"),
    files: {
      ".harness/eval.json": JSON.stringify({ assertions: [{ id: "E1" }, { id: "E2" }] }),
    },
  });
  ...
  assert.equal(statusOf(report, "eval assertion ids"), "ok");
});

test("doctor flags a prompt override whose name matches no prompt", async () => {
  const root = await makeProject({ specBody: VALID_SPEC });
  await mkdir(path.join(root, ".harness", "prompts"), { recursive: true });
  // The v3 rename left this name behind: resolved by name, so nothing reads it.
  await writeFile(path.join(root, ".harness", "prompts", "repair-semantic.md"), "stale\n");
  ...
  assert.equal(check.status, "warn");
  assert.match(check.detail, /repair-semantic\.md/);
  assert.match(check.fix, /repair-eval/);
});

test("doctor does not flag a live prompt override as stale", async () => {
  const root = await makeProject({ specBody: VALID_SPEC });
  await mkdir(path.join(root, ".harness", "prompts"), { recursive: true });
  await writeFile(path.join(root, ".harness", "prompts", "repair-eval.md"), "custom\n");
  ...
  const check = report.checks.find(c => c.name === "prompts");
  assert.equal(check.status, "warn");
  assert.match(check.detail, /1 override\(s\): repair-eval/);
});
