...
test("importPlaywright succeeds from the harness runtime dependency", async () => {
...
test("Playwright install guidance still names the package manager", () => {
...
test("package.json ships the SDK and Playwright as runtime dependencies", async () => {
  ...
  assert.equal(pkg.peerDependencies?.playwright, undefined);
  assert.equal(pkg.peerDependencies?.["@hiero-ledger/sdk"], undefined);
  assert.equal(pkg.peerDependenciesMeta?.playwright, undefined);
